...
function DisplayItems(props){
    ...
    const handleSelectAnnouncement = (announcementId) => {
        ...
        axios.post(`http://localhost:8000/api/auth/display/edit/${props.id}`, data,{
            headers: {
    ...
    return(
        <Card variant="outlined" alignItems="center" style={{width: '80%', marginBottom: '15px', marginLeft:'10%'}}>
            ...
            <CardActions>
                <Button size="small" aria-controls="simple-menu" aria-haspopup="true" onClick={handleClick}>Set Display</Button>
                <Menu
                    id={props.id}
                    anchorEl={anchorEl}
                    keepMounted
...
class DisplayList extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('http://localhost:8000/api/auth/announcement', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then(response => {
    ...
    <DashboardTemplate>
            ...
            {this.state.display.map(item => (
                <DisplayItems  key={item.id} name={item.name} id={item.id} announcement={this.state.announcement}/>
        ))}
            </div>
    </DashboardTemplate>
        )
